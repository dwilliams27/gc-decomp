...
def build_campaign_orchestrator_prompt(
    ...
    *,
    ...
    return (
        ...
        "- At campaign start and whenever the queue is mostly empty, bias strongly toward low-match or unattempted functions in the file. A couple of near-match cleanup attempts are fine, but do not let the campaign spend most of its time on the same 95%+ functions.\n"
        "- Treat low-percentage functions as discovery opportunities. A single breakthrough on types, control flow, or headers can unlock many later functions.\n"
        "- Use the very high-percentage functions more selectively: dip into them when you want a likely quick win, when a low-match exploration exposed a shared blocker, or when capacity remains after seeding lower-match work.\n"
